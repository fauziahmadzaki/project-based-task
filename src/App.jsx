      ...
      <div className="max-w-5xl mx-auto w-full relative z-10">
        <div className="text-center mb-16">
          ...
          <p className="text-lg text-slate-400 max-w-2xl mx-auto font-light">
            Kami adalah kelompok mahasiswa yang bersemangat dalam membangun
            antarmuka web modern yang menginspirasi. Kenali setiap anggota lebih
            dekat!
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {teamMembers.map((member, index) => (
            ...
            >
              <div
                className={`absolute inset-0 opacity-0 group-hover:opacity-10 bg-gradient-to-br ${member.color} transition-opacity duration-500`}
              ></div>

              <div className="relative mb-6">
                <div
                  className={`absolute inset-0 bg-gradient-to-br ${member.color} rounded-full blur-md opacity-40 group-hover:opacity-70 transition-opacity duration-500`}
                ></div>
                <img
                  className={`w-28 h-28 rounded-full bg-slate-800 border-[4px] border-slate-900 flex items-center justify-center text-3xl font-black text-white relative z-10 bg-gradient-to-br ${member.color} shadow-lg group-hover:scale-105 transition-transform duration-500`}
                  src={member.avatar}
                ></img>
              </div>
              ...
              <p className="text-slate-400 mb-6 font-medium">{member.role}</p>
